...
struct ActivityRepositoryTests {
    ...
    @Test
    func testSharedCompetitionSyncFailurePreservesCachedRemoteEntries() async throws {
        let day = calendar.startOfDay(for: Date())
        let suiteName = defaultsSuiteName()
        let defaults = isolatedDefaults(suiteName: suiteName)
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let wifeEntry = CompetitionEntry(
            competitor: CompetitorProfile(displayName: "Tiffany"),
            dayKey: ActivityFormatting.dayKey(for: day, calendar: calendar),
            steps: 8_500,
        ...
        let competitionSync = FakeSharedCompetitionSync(remoteEntries: [wifeEntry], failAfterCallCount: 1)
        let repository = ActivityRepository(
            healthKit: FakeHealthKitProvider(
                hourlyBuckets: [bucket(day, hour: 9, steps: 4_000, distance: 2_800, energy: 160)],
                dailyBuckets: [bucket(day, hour: 0, steps: 4_000, distance: 2_800, energy: 160)],
                workouts: []
            ),
            cloudKit: FakeCloudKitSummarySync(state: .available),
            competitionSync: competitionSync,
        ...
        await repository.updateSharedCompetition(isEnabled: true, inviteCode: "WIFE")
        #expect(repository.sharedCompetitionEntries.contains { $0.competitor.displayName == "Tiffany" })
        #expect(repository.competeBoardPhase == .active)

        await repository.syncSharedCompetition()
        #expect(repository.sharedCompetitionEntries.contains { $0.competitor.displayName == "Tiffany" })
        if case .unavailable = repository.sharedCompetitionSyncState {
            #expect(Bool(true))
        } else {
            Issue.record("Expected unavailable sync state after retry failure")
        }
    }
    ...
    func testSharedCompetitionProfileUpdatePersistsDisplayName() async throws {
        ...
        #expect(repository.preferences.displayName == "Tyron")
        #expect(repository.sharedCompetitionSettings.inviteCode == "FAMILY")
    }
...
private actor FakeSharedCompetitionSync: SharedCompetitionSyncing {
    ...
    init(
        remoteEntries: [CompetitionEntry] = [],
        syncError: Error? = nil,
        failAfterCallCount: Int? = nil
    ) {
    ...
    func sync(entries: [CompetitionEntry], inviteCode: String) async throws -> [CompetitionEntry] {
        ...
        if let failAfterCallCount, publishedBatches.count > failAfterCallCount {
            throw CloudSyncError.unavailable("Competition sync offline")
        }
        if let syncError {
            throw syncError
        }
